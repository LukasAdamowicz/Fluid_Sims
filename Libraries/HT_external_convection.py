...
class VerticalPlate(object):
    """
    Nusselt number correlations for vertical plates
    
    name=extconv.VerticalPlate(g,beta,Ts,Tinf,nu,Pr)
    where 
    ...
    name.local(x) for Nusselt number at an x location >0 above the bottom of the plate
    name.Gr_x Grashof number at location
    name.Nu_x Nusselt number at location
    
    name.average(L) for average Nusselt number over whole plate
    name.Gr_L Average Grashof number
    name.Nu_avg Average Nusselt number
    """
    def __init__(self,g,beta,Ts,Tinf,nu,Pr):
        self.g=g
        self.beta=beta
        self.Ts=Ts
        self.Tinf=Tinf
        self.nu=nu
        self.Pr=Pr
        self.gPr=(0.75*self.Pr**.5)/(0.609+1.221*self.Pr**.5+1.238*self.Pr)**.25
        
    def local(self,x):
        self.x=x
        if self.x<0:
            print('Warning: x must be larger than zero')
        self.Gr_x=(self.g*self.beta*abs(self.Ts-self.Tinf)*self.x**3)/self.nu**2
        if self.Gr_x*self.Pr>10**9:
            print("Warning: Rayleigh number too high for this correlation!")
        self.Nu_x=(self.Gr_x/4)**.25*self.gPr
        
    def average(self,L):
        self.L=L
        if self.L<0:
            print("Warning: L must be larger than zero")
        self.Gr_L=(self.g*self.beta*abs(self.Ts-self.Tinf)*self.L**3)/self.nu**2
        if self.Gr_L*self.Pr>10**9:
            print("Warning: Rayleigh number too high for this correlation!")
        self.Nu_avg=(4/3)*(self.Gr_L/4)**.25*self.gPr
